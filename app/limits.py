...
a token bucket per caller and a second one for the whole process, refilled over
time. Nothing distributed — Cloud Run is capped at a few instances, and the
point is to bound a bad afternoon, not to be exact.
"""
...
class Limiter:
    """One global ceiling, plus a smaller one per caller."""

    def __init__(self, per_caller: tuple[float, float], overall: tuple[float, float]):
        self._lock = threading.Lock()
        self._callers: dict[str, Bucket] = {}
        self._overall = Bucket(*overall)
    ...
    def check(self, caller: str, now: float | None = None) -> tuple[bool, int]:
        ...
        with self._lock:
            ...
            if not self._overall.take(now):
                b.tokens += 1                     # do not charge for a global refusal
                return False, self._overall.retry_after()
            return True, 0


# An autopsy is ~6 model calls: 5 per caller per hour, 60 across the service.
autopsy = Limiter(per_caller=(5, 5 / 3600), overall=(60, 60 / 3600))
# A sweep can be ten autopsies. Scheduler needs 4/hour; leave a little headroom.
sweep = Limiter(per_caller=(8, 8 / 3600), overall=(12, 12 / 3600))


def demo():
    """python -m app.limits — a ceiling that does not hold is not a ceiling."""
    lim = Limiter(per_caller=(2, 1 / 60), overall=(3, 1 / 60))
    t = 1000.0
    ...
    ok, _ = lim.check("c", t)
    assert not ok, "the global ceiling stops everyone once it is spent"
    ...
    print("OK — per-caller and overall ceilings both hold, and refill")
