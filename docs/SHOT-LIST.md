# Demo shot list — 3:48 target

**Preparation:** use only rows carrying the approved synthetic marker. Start on
the Task chat in light mode at desktop width. Keep the browser clock in
`Asia/Jerusalem`. Do not show tokens, project credentials, private raw events, or
Avi's pre-existing Notion row.

| Time | Shot | Visible proof | Spoken point |
|---|---|---|---|
...
| 2:48–3:05 | Run Knowledge cleanup again | “No dream notes to consolidate.” | No-work is decided before Gemini, so the second run makes no model call. |
| 3:05–3:27 | Open Learning; toggle day/week/month | Aggregate figures and daily summaries move | Learning is useful in the UI without exposing its private raw event log. |
| 3:27–3:40 | Reload Task and automation channels | Transcript and automation history return | Firestore and the knowledge volume preserve state across service reload. |
| 3:40–3:48 | Architecture diagram | One ADK agent, Vertex, scoped Notion MCP, Firestore, Cloud Storage | “One assistant, narrow permissions, honest defaults.” |

Total planned runtime: **3 minutes 48 seconds**. This is a recording plan only;
recording and publication require fresh, separate approval.
